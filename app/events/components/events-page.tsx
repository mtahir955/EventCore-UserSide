...
export function EventsPage() {
  ...
  // If you want, you can remove this too because apiClient already injects token in your project.
  const getToken = () => {
    let raw =
  ...
  const fetchBuyerEvents = async () => {
    try {
      ...
      setApiEvents([]); // no dummy fallback
    } finally {
      setLoading(false);
    }
  };
  ...
  // Build filter options from API only
  const marketOptions = useMemo(() => {
    const map = new Map<string, string>(); // slug -> label
    apiEvents.forEach((ev) => {
      const slug = ev?.marketSlug;
      const label = ev?.market;
      if (slug && label) map.set(slug, label);
    });
  ...
  const filteredEvents = useMemo(() => {
    return apiEvents.filter((event) => {
      ...
      const matchesStatus =
        filters.status === "all" ? true : mappedStatus === filters.status;
      ...
      <main className="flex-1 max-w-[1440px] mx-auto w-full px-4 sm:px-6 md:px-8 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4 mb-6 sm:mb-8">
          ...
          >
            ...
            <SelectContent className="bg-white dark:bg-[#1a1a1a] text-black dark:text-gray-100 border border-gray-200 dark:border-gray-700 max-h-64 overflow-y-auto">
