...
export function Header() {
  ...
  useEffect(() => {
    ...
    if (buyer || user || host) setIsLoggedIn(true);
      ...
      <header className="w-full bg-white border-b border-gray-200 dark:bg-[#101010] dark:border-[#333] dark:text-gray-100 transition-colors duration-300">
        <div className="max-w-[1440px] mx-auto flex items-center justify-between py-3 px-2 sm:px-6 md:px-8">
          ...
          >
            ...
            {/* Desktop SIGN UP Button (Hide if logged in) */}
            {/* Desktop SIGN UP Button (Hide if logged in) */}
            {!isLoggedIn && (
              <Link href="/sign-up">
                <Button className="hidden lg:inline-flex bg-[#ffffff] border border-[#0077F7] text-[#0077F7] hover:bg-[#eef6ff] px-6 py-2 rounded-full font-medium">
                  Sign Up
                </Button>
              </Link>
            )}

            {/* Desktop LOGIN Button (Hide if logged in) */}
            {!isLoggedIn && (
              <Link href="/login">
                <Button className="hidden lg:inline-flex bg-[#0077F7] hover:bg-[#0066D6] text-white px-6 py-2 rounded-full font-medium">
                  Login
                </Button>
              </Link>
            )}
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden bg-white dark:bg-[#2A2A2A] border-t border-gray-100 dark:border-[#333] px-5 py-4 space-y-3">
            ...
            {!isLoggedIn && (
              <Link href="/sign-up">
                <Button className="w-full mb-2 bg-white border border-[#0077F7] text-[#0077F7] hover:bg-[#eef6ff] rounded-full">
                  Sign Up
                </Button>
              </Link>
            )}
...
//code before hiding the login button
